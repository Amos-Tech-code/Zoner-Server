package com.amos_tech_code.data.database.models

import com.amos_tech_code.domain.model.AuthProvider
import com.amos_tech_code.domain.model.RegistrationStage
import com.amos_tech_code.domain.model.UserRole
import org.jetbrains.exposed.sql.Column
import org.jetbrains.exposed.sql.Table
import org.jetbrains.exposed.sql.javatime.datetime
import java.time.LocalDateTime
import java.time.LocalDateTime.now
import java.util.*

object UsersTable : Table("users") {
    // Core Identity
    val id: Column<UUID> = uuid("id").autoGenerate()
    val email: Column<String> = varchar("email", 255).uniqueIndex()
    val name: Column<String?> = varchar("name", 100).nullable()
    // Authentication
    val authProvider: Column<AuthProvider> = enumerationByName("auth_provider", 20, AuthProvider::class)
        .default(AuthProvider.EMAIL)
    val passwordHash: Column<String?> = varchar("password_hash", 255).nullable()
    // Profile Data
    val username: Column<String?> = varchar("username", 30).nullable().uniqueIndex()
    val profilePicUrl: Column<String?> = text("profile_pic_url").nullable()
    // Email Verification
    val isEmailVerified: Column<Boolean> = bool("is_email_verified").default(false)
    val emailVerifiedAt: Column<LocalDateTime?> = datetime("email_verified_at").nullable()
    // Registration Progress
    val registrationStage: Column<RegistrationStage> =
        enumerationByName("registration_stage", 20, RegistrationStage::class)
            .default(RegistrationStage.EMAIL_SUBMITTED)
    // Roles
    val role: Column<UserRole> = enumerationByName("role", 20, UserRole::class)
        .default(UserRole.USER)
    // Timestamps
    val createdAt: Column<LocalDateTime> = datetime("created_at").clientDefault { now() }
    val updatedAt: Column<LocalDateTime> = datetime("updated_at").clientDefault { now() }
    val lastLoginAt: Column<LocalDateTime?> = datetime("last_login_at").nullable()
    // Status Flags
    val isActive: Column<Boolean> = bool("is_active").default(true)
    val isBanned: Column<Boolean> = bool("is_banned").default(false)
    // Device Integration
    val fcmToken: Column<String?> = varchar("fcm_token", 255).nullable()

    override val primaryKey = PrimaryKey(id)

    init {
        // Additional performance indexes
        index(isUnique = false, columns = arrayOf(role)) // For role-based queries (BUSINESS/USER)
        index(isUnique = false, columns = arrayOf(isActive)) // For active user filtering
        index(isUnique = false, columns = arrayOf(createdAt)) // For user registration trends
        index(isUnique = false, columns = arrayOf(lastLoginAt)) // For user activity
        index(isUnique = false, columns = arrayOf(isBanned)) // For moderation queries
        index(isUnique = false, columns = arrayOf(email)) // Already unique, but ensure performance
        index(isUnique = false, columns = arrayOf(username)) // Already unique, but ensure performance
    }
}


object BusinessProfilesTable : Table("business_profiles") {
    val id = uuid("id").autoGenerate()
    val userId = (uuid("user_id") references UsersTable.id).uniqueIndex()
    val businessName = varchar("business_name", 255)
    val businessEmail = varchar("business_email", 255).nullable()
    val businessPhone = varchar("business_phone", 50).nullable()
    val businessAddress = text("business_address").nullable()
    val category = varchar("category", 100).nullable()
    val country = varchar("country", 100).nullable()
    val businessDescription = text("business_description").nullable()
    val businessLogo = text("business_logo").nullable()
    val websiteUrl = text("website_url").nullable()
    val followerCount = integer("follower_count").default(0)
    val isVerified = bool("is_verified").default(false)
    val verificationRequestedAt = datetime("verification_requested_at").nullable()
    val verifiedAt = datetime("verified_at").nullable()
    val taxId = varchar("tax_id", 100).nullable()
    val isTermsAccepted = bool("is_terms_accepted").default(false)
    // Timestamps
    val createdAt = datetime("created_at").clientDefault { now() }
    val updatedAt = datetime("updated_at").clientDefault { now() }

    override val primaryKey = PrimaryKey(id)

    init {
        // Additional performance indexes
        index(isUnique = false, columns = arrayOf(category)) // For category-based discovery
        index(isUnique = false, columns = arrayOf(country)) // For location-based queries
        index(isUnique = false, columns = arrayOf(isVerified)) // For verified business filtering
        index(isUnique = false, columns = arrayOf(followerCount)) // For popular business queries
        index(isUnique = false, columns = arrayOf(createdAt)) // For new business discovery
        index(isUnique = false, columns = arrayOf(userId)) // Already unique, but ensure performance
    }
}


object BusinessFollowersTable : Table("business_followers") {
    val id = uuid("id").autoGenerate()
    val followerId = uuid("follower_id") references UsersTable.id
    val followedUserId = uuid("followed_user_id") references UsersTable.id
    val followedAt = datetime("followed_at").clientDefault { now() }
    val isMuted = bool("is_muted").default(false)
    val isFavorite = bool("is_favorite").default(false)
    val relationshipScore = integer("relationship_score").default(50) // 0-100 scale

    override val primaryKey = PrimaryKey(id)

    init {
        uniqueIndex(followerId, followedUserId)
        index(isUnique = false, columns = arrayOf(followerId, followedAt))
        index(isUnique = false, columns = arrayOf(followedUserId, followedAt))

        // Additional performance indexes
        index(isUnique = false, columns = arrayOf(followerId)) // For follower-centric queries
        index(isUnique = false, columns = arrayOf(followedUserId)) // For followed-user-centric queries
        index(isUnique = false, columns = arrayOf(followedAt)) // For time-based analytics
        index(isUnique = false, columns = arrayOf(isMuted)) // For mute filtering
        index(isUnique = false, columns = arrayOf(isFavorite)) // For favorite features
        index(isUnique = false, columns = arrayOf(relationshipScore)) // For relationship-based sorting
    }
}